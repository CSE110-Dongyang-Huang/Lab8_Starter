describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 1;i<prodItems.length;i++){
      let data = await prodItems[i].getProperty('data');
      let val = await data.jsonValue();
      if (val.title.length == 0) { allArePopulated = false; }
      if (val.price.length == 0) { allArePopulated = false; }
      if (val.image.length == 0) { allArePopulated = false; }
    }
    expect(allArePopulated).toBe(true);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let index = 1; index < prodItems.length; index++) {
      let shadowRoot = await prodItems[index].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      await button.click();
    }
